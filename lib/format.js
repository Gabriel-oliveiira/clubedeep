...
export function labelPeriodicidade(p) {
  const m = { unico: 'Único', mensal: 'Mensal', anual: 'Anual', automatico: 'Automático' };
  return m[p] || 'Único';
}
// Forma de entrega do beneficio (quem faz acontecer)
export function labelFormaEntrega(f) {
  const m = { automatico: 'Automático', resgate_cliente: 'Resgate pela cliente', equipe: 'Entregue pela equipe' };
  return m[f] || 'Entregue pela equipe';
}
